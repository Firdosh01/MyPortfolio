                ...
                <div className='grid w-full gap-8 px-12 py-8 text-center sm:grid-cols-2 md:grid-cols-3 sm:px-0'>
                    {
                        frontend.map(({ id, src, title, style }) => (
                            <div key={id} className={`shadow-md hover:scale-105 duration-500 py-4 rounded-lg dark:bg-transparent bg-slate-100 ${style}`}>
                ...
                <div className='grid w-full gap-8 px-12 py-8 text-center sm:grid-cols-2 md:grid-cols-3 sm:px-0'>
                    {
                        backend.map(({ id, src, title, style }) => (
                            <div key={id} className={`shadow-md hover:scale-105 duration-500 dark:bg-transparent bg-slate-100 py-4 rounded-lg ${style}`}>
                                <img src={src} alt={title} className='w-20 mx-auto mb-3' />
                                <p className='text-black dark:text-white'> {title}</p>
                            </div>
                ...
                <div className='grid w-full gap-8 px-12 py-5 text-center sm:grid-cols-2 md:grid-cols-3 sm:px-0'>
                    {
                        otherskill.map(({ id, src, title, style, width }) => (
                            <div key={id} className={`shadow-md hover:scale-105 bg-slate-100 dark:bg-transparent duration-500 py-4 rounded-lg ${style}`}>
